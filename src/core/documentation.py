...
from src.core.language_support import Language, detect_language

logger = setup_logger("CodeCraftAI.documentation")
...
class DocumentationGenerator:
    ...
    def _generate_readme(self) -> str:
        ...
        """
        lines = []
        lines.append("# DevForgeAI")
        lines.append("")
        lines.append("**DevForgeAI** 是一个基于 LangGraph 和 Docker 的多智能体编程框架。")
        lines.append("你只需要描述需求，它就能自动完成规划、编码、测试的完整流程，帮你把想法变成可运行的代码。")
        lines.append("")
        lines.append("## 核心特性")
        lines.append("")
        ...
        lines.append("|-------|------|")
        lines.append("| **Planner** (架构师) | 理解需求，探索工作区，制定分步开发计划 |")
        lines.append("| **Coder** (工程师) | 精准执行代码修改，作为\"代码手术刀\" |")
        ...
        lines.append("")
        lines.append("```bash")
        lines.append("git clone https://github.com/你的用户名/DevForgeAI.git")
        lines.append("cd DevForgeAI")
        lines.append("pip install -r requirements.txt")
        lines.append("```")
        lines.append("")
        lines.append("### 配置")
        lines.append("")
        lines.append("在 `src/core/` 下创建 `.env` 文件（参考 `.env.example`）：")
        lines.append("")
        lines.append("```bash")
        ...
        lines.append("│   │   ├── Coder.py            # 工程师：代码编写 + 文件修改")
        ...
        lines.append("---")
        lines.append("")
        lines.append("Made by [LiHua](https://github.com/MagicalLiHua)")
    ...
    def _generate_api_docs(self) -> str:
        ...
        # 提取核心模块的 API
        core_dir = os.path.join(self.project_root, "src", "core")
    ...
    def _generate_architecture_docs(self) -> str:
        ...
        lines.append("    planner -- 无调用 --> coder[Coder]")
        lines.append("    coder -- 工具调用 --> step_counter[Step Counter]")
        lines.append("    step_counter --> coder_tools[Coder Tools]")
        lines.append("    coder_tools --> coder")
        lines.append("    coder -- 无调用 --> sandbox[Sandbox]")
        lines.append("    sandbox -- 通过 --> END_NODE(结束)")
        lines.append("    sandbox -- 失败 --> reviewer[Reviewer]")
        lines.append("    reviewer --> coder")
        ...
        lines.append("| `route_after_planner` | 无 tool_calls | `coder` |")
        lines.append("| `route_after_planner` | tool_calls ≥ MAX_PLANNER_STEPS | `coder` (强制结束探索) |")
        lines.append("| `route_after_coder` | 有 tool_calls | `coder_step_counter` → `coder_tools` |")
        lines.append("| `route_after_coder` | 无 tool_calls 或达到步数上限 | `sandbox` |")
        lines.append("| `route_after_sandbox` | 无 error_trace | `END` |")
        lines.append("| `route_after_sandbox` | 有 error 且 retry < max | `reviewer` → `coder` |")
